...
class VectorCompare:
    def magnitude(self, concordance):
        ...
        total = 0
        for word, count in concordance.items():  # Python 3: iteritems → items
            total += count ** 2
    ...
    def relation(self, concordance1, concordance2):
        ...
        topvalue = 0
        for word, count in concordance1.items():  # Python 3: iteritems → items
            if word in concordance2:  # Python 3: has_key → in
    ...
    def concordance(self, document):
        ...
        for word in document.split(' '):
            if word in con:  # Python 3: has_key → in
                con[word] = con[word] + 1
            else:
                con[word] = 1
        return con
...
while True:
    ...
    for i in range(len(index)):
        ...
        if relation != 0:
            # 显示文档标题（从内容中提取前50字符作为标题提示）
            title = documents[i][:50].replace('\n', ' ')
            matches.append((relation, title, documents[i][:100]))
    ...
    print("\n搜索结果（相关度从高到低）：")
    print("-" * 60)
    for relation, title, preview in matches:
        print(f"相关度: {relation:.4f}")
        print(f"文档开头: {title}...")
        print(f"预览: {preview}...")
        print()